...
public class DB {
	...
	public static String FILEPATH = "C:\\Users\\akkaurdr\\Desktop\\ATLAS_Java\\DMATAccountManager\\src\\main\\java\\com\\amazon\\dmataccountmanager\\db\\dbconfig.txt";
	...
	private void createConnection() {
		try {
			...
			if(file.exists()) {
				...
				buffer.close();
				reader.close();
				System.out.println("[DB] DB configured using File");
			}
			else {
				System.err.println("[DB] Can't read the db config file..");
			}
			connection =DriverManager.getConnection(URL+";user="+USER+";password="+PASSWORD+";integratedSecurity=false;" +
                    "encrypt=true;trustServerCertificate=true");
			System.out.println("[DB] connection created..");
		} catch (Exception e) {
			System.out.println("[DB] Something went wrong in creating the connection: "+e);
		}
	}
