...
public class RouteService {
	...
	public void deleteRoute() {
		...
		String sql = "Select * from Route where id="+route.id;
		List<Route> routeDetails = routeDAO.retrieve(sql);
		...
		if(routeDetails.size()>0) {
			...
			List<Stop> stopDetails = stopdao.retrieve(sql);
			
			if(stopDetails.size()>0) {
				for(Stop object:stopDetails) {
					stopdao.delete(object);
				}
			}else {
				System.out.println("No Stops on this route");
			}
			
			
			sql = "Select * from Vehicle where routeID="+route.id;
			List<Vehicle> vehicleDetails = vehicleDAO.retrieve(sql);
			
			if(vehicleDetails.size()>0) {
				for(Vehicle object:vehicleDetails) {
